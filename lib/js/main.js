        ...
        this.button.addEventListener("click", (e) => {
            ...
            if(this.clickCount < 1) {
                ...
                input.addEventListener("keyup", (e) => {
                    ...
                    console.log(this.accordianList[0].button.innerText.substring(0, matchString.length))
                    console.log(this.accordianList[0].button.innerText.toLowerCase())
                    console.log("a" !== "a");
                    for(let i = 0; i < this.accordianList.length; i++) {
                        console.log("Index: "+i);
                        if(matchString.length === 0) {
                            this.accordianList[i].button.classList.remove("hidden");
                        } else if(this.accordianList[i].button.innerText.substring(0, matchString.length).toLowerCase() !== matchString.toLowerCase) {
                ...
                fetch(url + urlEnd)
                    .then(res => res.json())
                    .then(res => {
                        for(let i = 1; i <= res.results.length; i++) { //fetches all data, makes them into accordian objects and appends them to the div
                            fetch(url + urlEnd + "/"+ i) 
                                .then(res => res.json())
                                .then(res => {
