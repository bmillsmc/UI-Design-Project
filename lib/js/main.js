            ...
            for(let j = 1; j <= infoList.length; j++) { //loops through each object in the array returned by the api and uses the data to populate the tab clicked
                fetch(url + urlEnd + "/"+j) //grab the next object in the api
                    ...
                    .then(res => { // the meat of the for loop where the processing of data and populating is done
                        ...
                        if(charDatTabs[i].childNodes.length <= divLengths[i] && charDatTabs[i].classList.contains("abil-sco-tab")){ //if the tab is the ability score tab then the data is processed throught this code
                            setTimeout(() => {
                                console.dir(res);
                        ...
                        } else if(charDatTabs[i].childNodes.length <= divLengths[i] && charDatTabs[i].classList.contains("skill-tab")) { //if the tab is the skills tab the data is proccessed throught this code
                            setTimeout(() => {
                                console.dir(res);
                            }, 20 + (j*20));
                        } else if(charDatTabs[i].childNodes.length <= divLengths[i] && charDatTabs[i].classList.contains("prof-tab")) { //there are 120 proficiencies, make this into an alphabetized list with a search bar that will real time narrow the list. Maybe make each item accordian if you have time
                            setTimeout(() => {
                                console.dir(res);
                            }, 20 + (j*20));
                        } else if(charDatTabs[i].childNodes.length <= divLengths[i] && charDatTabs[i].classList.contains("lang-tab")) { //if the tab is the languages tab the data is processed through this code
                            setTimeout(() => {
                                console.dir(res);
                            }, 20 + (j*20));
