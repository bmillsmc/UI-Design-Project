...
class Tab{
    constructor(button, div, main) { //button is a button element, div is the div related to the button, main is a boolean for whether its a main tab, and initilizes tabArray
        this.button = button;
        this.div = div;
        this.main = main;
        this.hide();
    ...
    clickEvent(e) {
        ...
        console.log("clickevent",this.div);
        // this.div.classList.remove("hidden");
    }
}

class ListTab extends Tab {
    constructor(button, div, main, tabName) {
        super(button, div, main);
        this.tabName = tabName;
        this.button.removeEventListener("click", this.clickEvent);
    }

    addClickEvent() { //to be done after appending tab array
        this.button.addEventListener("click", this.clickEventPopulate);
    }

    clickEventPopulate(e) {
        e.preventDefault();
        if(this.tabArray !== undefined) {
            for(let i = 0; i < this.tabArray.length; i++) {
                this.tabArray[i].hide();
        ...
        console.log("clickeventpopulate",this.div);
        // this.div.classList.remove("hidden");
        ...
        fetch(url + urlEnd)
            .then(res => res.json())
            .then(res => {
                for(let i = 0; i < /*res.results.length*/20; i++) {
                    fetch(url + urlEnd + "/"+ i)
                        .then(res => res.json())
                        .then(res => {
                            console.dir(res);
...
let spellsTab = new ListTab(mainButs[4], mainTabs[4], true, mainButs[4].innerText);
...
setTimeout(() => {
    ...
    spellsTab.appendTabArray(mainTabObjects);
    spellsTab.addClickEvent();
}, 300);

class Accordian {
    constructor(title, div, parent){ //takes in a title for the accoridan which is displayed when its closed and an unappended div with the contents of the accordian, and the parent div to append the accordian to
